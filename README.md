...
This is the code for "Synergizing LLM Agents and Knowledge Graphs for Socioeconomic Prediction in LBSN"
# Step 1: Embedding LM finetuning on LBKG
...
bash run.sh
```
3. Get embeddings for entities in LBKG:
```
cd ..
...
1. Install neo4j, start the neo4j server and run the following command to load LBKG to neo4j server:
```
